...
validity_stats <- data.frame(year = year_list, 
                             ...
                             avg_saleprice = numeric(length(year_list)), 
                             Q1_saleprice = numeric(length(year_list)), 
                             Q3_saleprice = numeric(length(year_list)),
                             avg_pred_saleprice = numeric(length(year_list))
                             )
...
for (year_num in 1:length(year_list)) {
  ...
  # Calculate average tractor prices, along with interquartile range. 
  validity_stats[year_num, 'avg_saleprice'] <- mean(tractor_prod_year[, 'saleprice'])
  validity_stats[year_num, 'Q1_saleprice'] <- quantile(tractor_prod_year[, 'saleprice'], 0.25)
  validity_stats[year_num, 'Q3_saleprice'] <- quantile(tractor_prod_year[, 'saleprice'], 0.75)
  
  # Calculate average predicted tractor prices.
  validity_stats[year_num, 'avg_pred_saleprice'] <- mean(tractor_prod_year[, 'pred_saleprice'])
...
fig_file <- "Figures/pred_vs_actual_IQR.png"
png(fig_file)

y_min <- min(validity_stats[, 'Q1_saleprice'])
y_max <- max(validity_stats[, 'Q3_saleprice'])
plot(validity_stats[, 'year'], 
     validity_stats[, 'avg_pred_saleprice'], 
...
abline(h = validity_stats[1, 'avg_pred_saleprice'], 
       lty = 'dashed', lwd = 2)
# Add the original average observed values.
lines(validity_stats[, 'year'], 
      validity_stats[, 'avg_saleprice'], 
      lwd = 3, 
      col = 'red', 
)
# Add the upper and lower quartiles.
lines(validity_stats[, 'year'], 
      validity_stats[, 'Q1_saleprice'], 
      lwd = 3, 
      lty = 'dashed',
      col = 'red', 
)
lines(validity_stats[, 'year'], 
      validity_stats[, 'Q3_saleprice'], 
      lwd = 3, 
      lty = 'dashed',
      col = 'red', 
)
...
year_list_sel <- c(2000, 2003, 2004, 2005)
...
fig_file <- "Figures/lift_chart_2003_2005.png"
png(fig_file)
